...
- funding_stage — the CAP TABLE, which is a different question from stage above: stage asks "has it shipped", this asks "who has already priced it". One of "bootstrapped" | "pre_seed" | "seed" | "series_a" | "series_b" | "series_c_plus" | "unknown". Set it ONLY from an explicit announcement in the evidence — a funding post, a press release, an "backed by / raised / we closed" statement, an investor logo strip naming a round. Use "unknown" whenever the evidence does not state a round, which will be MOST companies: never infer a round from headcount, polish, traction, follower count or how established the company looks. A wrong round is worse than no round, because it decides whether the company is past the fund's entry point. "bootstrapped" only when they explicitly say so.
- funding_amount: the amount exactly as announced ("$10M", "£2.5M", "$1.4B"), else null. Never estimate or convert.
- funding_investors: named lead or notable investors stated in the evidence, else []. Names only — no guesses from an investor's follow.
- funding_evidence: REQUIRED whenever funding_stage is not "unknown" — quote or name where you read it, e.g. "bio: 'Bootstrapped founder'", "website: press page", "tweet 2026-03-11". A round with no funding_evidence is DISCARDED and reset to "unknown", so if you know the round, say where it came from. If the only reason you believe it is prior knowledge of the company rather than something in this dossier, that is exactly the case where the answer must be "unknown".
...
- IS THE FUNDING STAGE EVIDENCED? funding_stage must trace to an actual announcement in the dossier (a raise post, press page, "backed by" statement, investor logos naming a round). If the analyst set a round the evidence does not state — inferring "seed" from a small team or "series_a" from a polished site — correct it to "unknown" and say so. "unknown" is the right answer for most companies and is never a defect; an invented round is, because it decides whether this company is past the fund's entry point.
...
_CORRECTION_FIELDS = {
    ...
    "funding_stage", "funding_amount", "funding_investors",
}
